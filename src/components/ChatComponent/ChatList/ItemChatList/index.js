import React from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { setActiveChat, setActiveUser } from '~/redux/slides/Chat';

export const avatar =
  'data:image/png;base64,';

function ItemChatList({ chat, toggleChatList }) {
  const dispatch = useDispatch();
  const handleClickItem = () => {
    dispatch(setActiveChat(chat));
  };
  const activeUsers = useSelector((state) => state.chat.activeUsers);
  const activeChat = useSelector((state) => state.chat.activeChat);
  const isOnline = activeUsers.includes(chat.user?.id);
  return (
    <div
      onClick={handleClickItem}
      className={`border-b flex justify-between items-center px-5 py-2 min-h-[74px] rounded transition-all hover:cursor-pointer hover:bg-gray-100 ${
        chat.user?.id === activeChat.user?.id && 'bg-gray-100'
      }`}
    >
      <div className="flex justify-center items-center gap-5">
        <div className="flex rounded justify-center transition-all">
          <div className="flex items-center justify-center relative">
            <img
              className="w-16 h-16 rounded-full border border-gray-300"
              src={chat.user?.avatar || avatar}
              alt="avatar user"
            />
            {isOnline && <div className="w-3 h-3 rounded-full bg-green-600 absolute right-1 bottom-1"></div>}
            {!isOnline && <div className="w-3 h-3 rounded-full bg-red-700 absolute right-1 bottom-1"></div>}
          </div>
          {!toggleChatList && (
            <div className="text-[8px] ml-2 rounded-full text-white bg-red-500 inline-flex w-7 h-7 items-center justify-center">
              12
            </div>
          )}
        </div>
        {toggleChatList && (
          <div className={`flex flex-col justify-between ${toggleChatList ? 'max-w-full' : 'max-w-0'}`}>
            <span className="font-medium text-black">{chat.user?.fullName}</span>
            <span className="text-[14px]">{isOnline ? 'Đang hoạt động' : 'Ngoại tuyến'}</span>
          </div>
        )}
      </div>
      <div className={`flex flex-col items-end justify-center gap-y-1 ${toggleChatList ? 'max-w-full' : 'max-w-0'}`}>
        {toggleChatList && (
          <>
            <div className="text-[8px] rounded-full text-white bg-red-500 inline-flex w-7 h-7 items-center justify-center">
              12
            </div>
            <span className="text-[14px]">30 phút trước</span>
          </>
        )}
      </div>
    </div>
  );
}

export default ItemChatList;
