...
function ItemChatList({toggleChatList, index, indexActive, setIndexActive}) {
  const handleClickItem = () => {
    setIndexActive(index);
  }
  return (
    <div onClick={handleClickItem} className={`border-b flex justify-center items-center px-5 py-2 min-h-[74px] rounded transition-all hover:cursor-pointer hover:bg-gray-100 ${indexActive === index && "bg-gray-100"}`}>
        ...
        </div>
        {
          toggleChatList && (
            <div className={`flex flex-col justify-between ${toggleChatList ? "max-w-full" : "max-w-0"}`}>
              <span className='font-medium text-black'>Phan Trọng Tính</span>
