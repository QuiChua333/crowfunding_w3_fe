...
function ItemChat({msg}) {
  return (
    <div className={`flex gap-4 max-w-full w-full ${msg.isGemini ? "justify-start" : "justify-end"}`}>
        ...
        <div className={`px-5 py-2 border text-white ${msg.isGemini ? "bg-[#2d4365]" : "bg-[#328ce1]"} inline-flex items-baseline flex-col gap-2 rounded-xl max-w-[70%] overflow-x-auto`}>
            <Markdown className={`max-w-full break-all text-wrap whitespace-wrap self-start text-[10px] md:text-[14px] text-start`}>{msg.content}</Markdown>
