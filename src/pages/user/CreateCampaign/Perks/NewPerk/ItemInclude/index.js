...
function ItemInclude({
  ...
  return (
    <div className={cx('wrapper')}>
      <div className="w-[80%]" style={{ padding: '6px' }}>
        ...
        <div className={cx('entreField-select')}>
          <a
            ...
            onClick={() => setShowListItemInclude((prev) => !prev)}
            ref={element}
          >
            <span>{itemData.name || 'Chọn một vật phẩm'}</span>

            <FaAngleDown className={cx('icon', 'icon-down')} />
            {showListItemInclude && (
              <div className={cx('dropdown-outer')}>
                <DropDown
                  listItemInclude={listItemsAvailable}
                  onClickItem={handleClickItemInclude}
                  index={index}
      ...
      <div className="w-[30%]" style={{ padding: '6px' }}>
