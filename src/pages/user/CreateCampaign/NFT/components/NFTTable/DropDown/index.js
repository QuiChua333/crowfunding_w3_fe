...
function DropDown({ menu, onClickItem, index, perk, changeFeatured, isEditComponent, handleCRemovePerk }) {
  ...
  return (
    <div className={cx('wrapper')} style={{ pointerEvents: !isEditComponent && 'none' }}>
      {perk.isVisible && !perk.isFeatured && (
        <div onClick={handleClickFeatured} className={cx('action')}>
          Đặt thành nổi bật
        </div>
      )}
      <div onClick={() => navigate(`/campaigns/${id}/edit/perks/${perk.id}`)} className={cx('action')}>
        Chỉnh sửa đặc quyền
      </div>
      {perk.isFeatured && (
        <div className={cx('action')} onClick={handleClickFeatured}>
          Dừng làm nổi bật
        </div>
      )}
