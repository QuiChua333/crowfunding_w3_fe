...
import { useUpdateFieldGroupMutation } from "~/hooks/api/mutations/admin/admin.fieldGroup.mutation";
import { setLoading } from "~/redux/slides/GlobalApp";

function ModalEdit({ setOpenModal, item, getAllFieldGroup}) {
  const dispatch = useDispatch();
    ...
    const updateFiledGroup = useUpdateFieldGroupMutation();
    const handleSave = async () => {
      if (input.trim().length > 0) {
        dispatch(setLoading(true));
        updateFiledGroup.mutate(
    ...
    <div className="fixed inset-0 z-[2000] flex justify-center bg-black bg-opacity-50" onClick={() => setOpenModal(false)}>
      <div className="w-1/2 bg-white rounded-lg shadow-lg p-8 mt-[100px] h-fit" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center border-b pb-2">
          <h2 className="text-[20px] font-medium">Chi tiết nhóm lĩnh vực</h2>
        ...
        <div className="mt-10 p-5">
          <label className="block text-[14px] font-medium text-gray-500">Nhóm lĩnh vực:</label>
          <input
            type="text"
            value={input}
