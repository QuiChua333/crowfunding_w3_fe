...
import { useCreateFieldMutation } from "~/hooks/api/mutations/admin/admin.field.mutation";
import { setLoading } from "~/redux/slides/GlobalApp";

function ModalAdd({ setOpenModal, getAllFieldByGroup}) {
  const dispatch = useDispatch();
    ...
    const addFiled = useCreateFieldMutation();
    const handleSave = async () => {
      if (input.trim().length > 0) {
        dispatch(setLoading(true));
        addFiled.mutate(
          { name: input, fieldGroupId: idFieldGroup },
          {
            onSuccess: (res) => {
              getAllFieldByGroup();
